...
// HTML 태그들을 JS에서 조작하기 위해 메모리 객체 주소를 변수에 저장합니다.
  ...
  function renderMiniCalendar() {
    ...
    miniCalendarTitle.textContent = monthNames[displayedMonth.getMonth()] + " " + displayedMonth.getFullYear();
  ...
  function renderWeekView() {
    ...
    if (selectedWeekStart.getMonth() === weekEnd.getMonth()) {
      weekTitle.textContent = startMonth + " " + selectedWeekStart.getDate() + " - " + weekEnd.getDate() + ", " + selectedWeekStart.getFullYear();
    } else {
      weekTitle.textContent = startMonth + " " + selectedWeekStart.getDate() + " - " + endMonth + " " + weekEnd.getDate() + ", " + weekEnd.getFullYear();
    }
    ...
    for (let i = 0; i < 7; i++) {
      ...
      //  각 열에 해당하는 날짜 계산 (월요일부터 시작하여 7일간)
      date.setDate(date.getDate() + i);
      // 헤더 셀 생성
      const headerCell = document.createElement('div');
      headerCell.className = 'grid-cell header';

      // 오늘 날짜인 경우 헤더 셀에 특별한 클래스 추가하여 강조
      if (isSameDay(date, currentDate)) {
        headerCell.classList.add('today-col');
      }

      // 요일 이름 인덱스 계산 (월요일부터 시작하므로 +1 보정)
      const dayNameIndex = (i + 1) % 7;
      
      // 헤더 셀에 요일과 날짜를 함께 표시 (예: "Mon 23")
    ...
    for (let hour = 9; hour <= 21; hour++) {
      ...
      for (let day = 0; day < 7; day++) {
        ...
        if (classForCell) {
          // 시작 분을 기반으로 top 위치 계산 (0분 = 0%, 30분 = 50%)
          const topOffset = (classForCell.startMinutes / 60) * 100;

          // 수업 시간(분 단위)을 계산하여 높이 결정
          ...
          classBlock.className = 'task-cell clickable ' + classForCell.colorClass;
          ...
          //  topOffset과 heightPercent를 이용하여 수업 블록의 위치와 크기를 설정합니다.
  ...
  function showDetailModal(schedule) {
    ...
    // 시간 문자열 생성
    const startTimeStr = schedule.startHour.toString().padStart(2, '0') + ':' +
                         schedule.startMinutes.toString().padStart(2, '0');
    const endTimeStr = schedule.endHour.toString().padStart(2, '0') + ':' +
                       schedule.endMinutes.toString().padStart(2, '0');

    // 상세 정보 표시
    detailStudentName.textContent = schedule.studentName;
    detailDate.textContent = schedule.date + ' (' + dayName + ')';
    detailTime.textContent = startTimeStr + ' - ' + endTimeStr;
  ...
  editBtn.addEventListener('click', function() {
    ...
    // 기존 모달에 데이터 채우기
    studentNameInput.value = selectedSchedule.studentName;

    // 날짜에서 요일 계산
    const dateObj = new Date(selectedSchedule.date);
    const dayIndex = dateObj.getDay(); // 0=일, 1=월, ...
    const dayMap = ['sun', 'mon', 'tue', 'wed', 'thu', 'fri', 'sat'];
    daySelectInput.value = dayMap[dayIndex];

    // 시간 설정
    startTimeInput.value = selectedSchedule.startHour.toString().padStart(2, '0') + ':' +
                           selectedSchedule.startMinutes.toString().padStart(2, '0');
    endTimeInput.value = selectedSchedule.endHour.toString().padStart(2, '0') + ':' +
                         selectedSchedule.endMinutes.toString().padStart(2, '0');
  ...
  deleteBtn.addEventListener('click', async function() {
    ...
    // 삭제 확인
    const confirmDelete = confirm(selectedSchedule.studentName + ' 수업을 삭제하시겠습니까?');
    if (!confirmDelete) return;

    try {
      const response = await fetch(`http://localhost:3000/api/schedules/${selectedSchedule.id}`, {
        method: 'DELETE'
      });

      if (response.ok === true) {
        // 삭제 성공: 모달 닫고 데이터 다시 로드
        detailModal.classList.remove('active');
        selectedSchedule = null;
  ...
  async function loadSchedulesFromServer() {
    try {
      // 1. 서버에 데이터를 요청합니다. (기본 메서드는 GET입니다.)
      const response = await fetch('http://localhost:3000/api/schedules');

      // 2. 응답이 성공적이지 않다면 에러를 발생시킵니다.
      if (response.ok === false) {
        throw new Error('데이터 수신 실패');
      }

      // 3. 문자열 데이터를 자바스크립트 객체로 역직렬화합니다.
      const data = await response.json();

      // 4. 전역 상태 변수인 classes 배열을 업데이트합니다.
      classes = data;

      // 5. 업데이트된 데이터를 바탕으로 화면을 다시 그립니다.
      renderWeekView();
    } catch (error) {
  ...
  submitBtn.addEventListener('click', async function() {
    ...
    const classDateStr = classDate.toISOString().split('T')[0]; // "YYYY-MM-DD" 형식
    ...
    try {
      ...
      if (selectedSchedule) {
        // 수정 모드: PUT 요청
        response = await fetch(`http://localhost:3000/api/schedules/${selectedSchedule.id}`, {
          method: 'PUT',
          headers: {
        ...
        // 생성 모드: POST 요청
        response = await fetch('http://localhost:3000/api/schedules', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json'
          },
      ...
      if (response.ok === true) {
        // 서버 저장 성공 시, 서버의 최신 데이터를 다시 불러옵니다.
        await loadSchedulesFromServer();
        ...
        endTimeInput.value = '';
        selectedSchedule = null; // 선택 상태 초기화
      } else {
        alert('서버 저장 실패');
      }
    } catch (error) {
